...
_sh = {"session": None, "crumb": None, "crumb_time": 0, "last_attempt": 0}

def get_session_and_crumb():
    now = time.time()
    if _sh["crumb"] and (now - _sh["crumb_time"] < CRUMB_TTL):
        return _sh["session"], _sh["crumb"]
    if (now - _sh["last_attempt"]) < CRUMB_RETRY_COOLDOWN:
        return _sh["session"], _sh["crumb"]
    _sh["last_attempt"] = now
    session = _sh["session"] or creq.Session(impersonate="chrome110")
    try:
    ...
    try:
        ...
        print(f"[crumb] status={r.status_code} body={r.text[:100]!r}")
        if r.status_code == 200 and r.text and "<html" not in r.text.lower() and len(r.text.strip()) < 40:
            _sh["crumb"] = r.text.strip()
            _sh["crumb_time"] = now
    except Exception as e:
...
def yahoo_get(url, params=None):
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"[yahoo] {url} status={r.status_code}")
            return None
        return r.json()
    except Exception as e:
        print(f"yahoo_get error {url}: {e}")
        return None
...
def get_raw_expiries(ticker):
    ...
    try:
        ...
        print(f"expiries error {ticker}: {e}")
        return []

def get_chain(ticker, unix_ts):
    ...
    try:
        ...
        print(f"chain error {ticker} {unix_ts}: {e}")
        return [], []
...
def api_expiries():
    ...
    for unix_ts in raw:
        ...
        if dte < 0 or dte > 365:
            continue
        calls, puts = get_chain(ticker, unix_ts)
        oi = sum(c.get("openInterest", 0) or 0 for c in calls) + sum(p.get("openInterest", 0) or 0 for p in puts)
        result.append({
            "dte": dte,
...
def api_gex():
    ...
    selected = expiries_param.split(",")
    spot = get_spot_price(ticker)
    if spot <= 0:
        return jsonify({"error": "Could not fetch spot price. Try again."}), 400
    today = datetime.now(IST).date()
    strike_map = {}
    for unix_str in selected:
        try:
            ...
            print(f"gex error {ticker} {unix_str}: {e}")
            continue
        for row in calls:
            K = float(row.get("strike", 0))
            oi = float(row.get("openInterest", 0) or 0)
            iv = float(row.get("impliedVolatility", 0.25) or 0.25)
            if oi <= 0 or K <= 0: continue
            ...
            s = strike_map.setdefault(K, {"gex": 0.0, "dex": 0.0, "cg": 0.0, "pg": 0.0})
            s["gex"] += gex; s["dex"] += dex; s["cg"] += gex
        for row in puts:
            K = float(row.get("strike", 0))
            oi = float(row.get("openInterest", 0) or 0)
            iv = float(row.get("impliedVolatility", 0.25) or 0.25)
            ...
            dex = delta * oi * CONTRACT_SIZE * spot
            s = strike_map.setdefault(K, {"gex": 0.0, "dex": 0.0, "cg": 0.0, "pg": 0.0})
            s["gex"] += gex; s["dex"] += dex; s["pg"] += gex
    if not strike_map:
        return jsonify({"error": "No open interest data found"}), 400
    ...
    call_wall = max(strike_map.items(), key=lambda kv: kv[1]["cg"])[0]
    put_wall = min(strike_map.items(), key=lambda kv: kv[1]["pg"])[0]
    cum = 0.0; cum_list = []; flip = strikes[0]; prev = None
    for k in strikes:
        cum += strike_map[k]["gex"]; cum_list.append(cum)
        if prev is not None and prev < 0 <= cum: flip = k
        prev = cum
    mp = strikes[0]; mp_cost = None
    for c in strikes:
        cost = sum(abs(strike_map[k]["gex"]) * abs(c - k) for k in strikes)
        if mp_cost is None or cost < mp_cost: mp_cost = cost; mp = c
    cum2 = 0.0; cdex = []
    for i, k in enumerate(strikes):
        cum2 += dex_vals[i]; cdex.append({"strike": k, "value": cum2})
    return jsonify({
        "ticker": ticker, "spot": round(spot, 2),
        "strikes": strikes, "gex": gex_vals, "dex": dex_vals,
        "cumulative_gex": [{"strike": strikes[i], "value": cum_list[i]} for i in range(len(strikes))],
        "cumulative_dex": cdex,
        "call_wall": call_wall, "put_wall": put_wall,
        "net_gex": round(net_gex, 2), "net_dex": round(net_dex, 2),
        "flip_point": flip, "max_pain": mp,
